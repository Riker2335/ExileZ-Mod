...
ExtendedLogging					= false;				// Slightly more information in logs.
MinSpawnDistance				= 20;               // Closest distance from any player to spawn a zombie.
MaxSpawnDistance				= 180;              // Max distance a zombie should spawn from a player.
MaxDistance						= 250;              // Max distance to players before delete.
MaxTime							= 10;               // Max time away from a player before delete.
...
ZombieMaxMoney					= 100;               // Max Money per zombie kill-random amount put on corpses
...
EZM_ryanzombiesinfectedrate               = 0.025;   // Damage per minute when infected (+/- 30 minutes to live @ 0.05)
...
EZM_ryanzombiesantivirusduration          = 600;    // Antivirus duration *(300 = 5 minutes)
...
HarassingLoopTime						= 300; // Check for players to harass every X seconds (Default 180 seconds)
HarassingConfig = [
